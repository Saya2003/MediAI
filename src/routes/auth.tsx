...
function AuthPage() {
  ...
  return (
    <div className="grid min-h-screen md:grid-cols-2">
      <div className="hidden flex-col justify-between bg-gradient-to-br from-primary/95 to-primary p-12 text-primary-foreground md:flex">
        <Link to="/" className="flex items-center gap-2">
          <CircleDot className="h-5 w-5" />
          <span className="font-display text-2xl">MediFlow AI</span>
        </Link>
        <div className="space-y-6">
          <h1 className="font-display text-5xl leading-tight">Coordinate care like a single, calm operating room.</h1>
          <p className="max-w-md text-primary-foreground/80">
            Every referral, every pre-auth, every appointment — orchestrated across humans, robots, and AI agents.
          </p>
        </div>
        <p className="text-xs text-primary-foreground/60">Synthetic data · Demonstration environment</p>
      </div>
      ...
      <div className="flex items-center justify-center p-8">
        <div className="w-full max-w-sm">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {mode === "signin" ? "Sign in to your coordinator workspace." : "New accounts default to the Coordinator role."}
          </p>
