...
export const Route = createFileRoute("/_authenticated/cases/")({
  head: () => ({ meta: [{ title: "Patient cases · MediFlow" }] }),
  errorComponent: ({ error }) => <div className="p-10 text-destructive">{error.message}</div>,
  notFoundComponent: () => <div className="p-10">No cases yet</div>,
  component: CasesPage,
});
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-8">
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="font-display text-4xl">Patient cases</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Every patient your team is helping, from new request to all done.
          </p>
        </div>
...
function CaseTable() {
  ...
  const filtered = useMemo(() => {
    return data.filter((c: any) => {
      ...
      if (
        q &&
        !`${c.patient_name} ${c.mrn} ${c.case_number} ${c.specialty}`
          .toLowerCase()
              ...
              return (
                <tr key={c.id} className="transition-colors hover:bg-muted/30">
                  ...
                  <td className="px-5 py-3">
                    ...
                    <div className="text-xs text-muted-foreground">Patient ID {c.mrn}</div>
                  </td>
