...
export const Route = createFileRoute("/_authenticated/cases/$caseId")({
  head: ({ params }) => ({
    meta: [{ title: `Case ${params.caseId.slice(0, 8)} · MediFlow` }],
  }),
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="font-mono text-xs text-muted-foreground">
              {c.case_number} · Patient ID {c.mrn}
            </div>
            <h1 className="font-display text-4xl">{c.patient_name}</h1>
            <p className="mt-1 text-sm text-muted-foreground">
              {c.specialty} · sent in by {c.referring_physician_name ?? "—"}
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <StatusPill tone={PRIORITY_TONE[c.priority]}>
              ...
              <div key={s}>
                <div
                  className={`h-1.5 rounded-full ${
                    done ? "bg-success" : current ? "bg-primary" : "bg-muted"
                  }`}
                />
                <div
        ...
        <div className="space-y-6 lg:col-span-2">
          ...
          <section className="surface-card p-6">
            ...
            <p className="text-sm text-muted-foreground">
              Everything that's happened on this case, in order.
            </p>
                ...
                <li key={e.id} className="flex gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <StatusPill tone={ACTOR_TONE[e.actor_type]}>
              ...
              <dl className="mt-3 space-y-2 text-sm">
                ...
                <Row
                  ...
                  value={
                    <StatusPill
                      tone={
                        data.preauths[0].status === "approved"
                          ? "success"
                          : data.preauths[0].status === "denied"
                          ? "danger"
                          : "warning"
                      }
                    >
            ...
            <section className="surface-card p-6">
              ...
              <dl className="mt-3 space-y-2 text-sm">
                ...
                <Row
                  label="When"
                  value={
                    data.appointments[0].scheduled_at
                      ? format(new Date(data.appointments[0].scheduled_at), "PP p")
                      : "—"
                  }
                />
    ...
    <div className="rounded-lg border border-border bg-background p-4 transition-shadow hover:shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          {task.description && (
            <div className="mt-0.5 text-sm text-muted-foreground">{task.description}</div>
          )}
        </div>
        {breached && (
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {isPreAuth ? (
          <>
            ...
            <Dialog open={open} onOpenChange={setOpen}>
              ...
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>Deny insurance approval</DialogTitle>
                </DialogHeader>
                <Textarea
        ...
        <Button size="sm" variant="ghost" onClick={() => act("escalate")} disabled={busy}>
          Ask supervisor
        </Button>
      </div>
    </div>
